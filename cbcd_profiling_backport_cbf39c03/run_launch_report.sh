#!/usr/bin/env bash
# Run an OpenSn regression test with Caliper's runtime-report service, then
# extract a flat CBCD-specific table from the raw report.
#
# This is the most useful script for answering:
#   - how many times each CBCD region is entered
#   - whether CBCD is paying for too many launches / scheduler steps
#   - whether host-side orchestration dominates kernel work
#
# Usage:
...
function print_header() {
  printf "%-56s %12s %12s %12s %12s %14s %14s %14s %14s %16s %16s\n", \
         ...
         "Calls/rk(avg)", "Calls/rk(max)", "Calls(total)", "Visits", \
         "Min(ns)", "Max(ns)"
}
