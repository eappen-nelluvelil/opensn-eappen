...
public:
  CBC_FLUDS(size_t num_groups,
            ...
            // std::vector<double>& local_psi_data, // get rid of this
  ...
  double*
  GetLocalDownwindPsi(const Cell& cell);
  ...
  const std::vector<double>& GetLocalPsiData() const
  {
    return local_psi_data_;
  }

  // Non-const getter (for writing into the subset)
  std::vector<double>& GetLocalPsiData()
  {
    return local_psi_data_;
  }
