...
CbcSweepChunk::Sweep(AngleSet& generic_angle_set) // Renamed param for clarity
{
    ...
    std::vector<Vector<double>> b_rhs(gs_size_, Vector<double>(max_num_cell_dofs_)); // RHS, b_rhs[gsg](node_i)
    std::vector<double> cell_source_for_angle(max_num_cell_dofs_); // q_moms projected for current angle
    ...
    const auto& sigma_t_xs = xs_.at(cell_->block_id)->GetSigmaTotal(); // Total cross-section vector for material

    // --- Loop over angles in the current AngleSet ---
    const std::vector<size_t>& global_angle_indices_in_set = current_cbc_angle_set.GetAngleIndices();

    for (size_t ang_loop_idx = 0; ang_loop_idx < global_angle_indices_in_set.size(); ++ang_loop_idx)
    {
        const size_t direction_num = global_angle_indices_in_set[ang_loop_idx]; // GLOBAL angle index
        const unsigned int angle_idx_as = current_cbc_angle_set.GetLocalAngleIndex(direction_num); // LOCAL index within AngleSet
        ...
        for (unsigned int f = 0; f < cell_num_faces_; ++f) {
            ...
            for (unsigned int fi = 0; fi < num_face_nodes; ++fi) { // Current face node on current cell's face
                const int i_local_cell = cell_mapping_->MapFaceNode(f, fi); // Map face node fi to cell node i_local_cell

                for (unsigned int fj = 0; fj < num_face_nodes; ++fj) { // Upstream face node on adjacent cell/boundary
                    const int j_local_cell = cell_mapping_->MapFaceNode(f, fj); // Map face node fj to cell node j_local_cell (for Amat)

                    const double mu_Nij_coeff = -face_mu_values[f] * M_surf_[f](i_local_cell, j_local_cell);
                    Amat(i_local_cell, j_local_cell) += mu_Nij_coeff;
                    ...
                    if (is_local_face) {
                      const unsigned int upwind_node_idx_in_neighbor_cell = face_nodal_mapping->cell_node_mapping_[fj];
                      const Cell* upwind_cell_ptr = cell_transport_view_->FaceNeighbor(f);
                      upwind_source_info = "LOCAL_UPWIND from cell " + (upwind_cell_ptr ? std::to_string(upwind_cell_ptr->global_id) : "NULL") +
                                            ", node_in_neigh=" + std::to_string(upwind_node_idx_in_neighbor_cell) +
                                            ", reading for angle_as=" + std::to_string(angle_idx_as);
                      psi_upwind_groups_ptr = fluds_->GetLocalUpwindPsi_Compact(
                          *upwind_cell_ptr,
                      ...
                      const unsigned int face_node_idx_in_mpi_buffer = face_nodal_mapping->face_node_mapping_[fj];
                      upwind_source_info = "MPI_UPWIND, mpi_face_node_idx=" + std::to_string(face_node_idx_in_mpi_buffer) +
                                           ", reading for angle_as=" + std::to_string(angle_idx_as);
                    ...
                    log.Log0Verbose1() << "SNIFF_TEST: Cell " << cell_->global_id << "  Angle_gidx=" << direction_num
                    << "    IncFace=" << f << " Node_on_face_fi=" << fi << " (maps to cell_node=" << i_local_cell << ")"
                    ...
                    if (psi_upwind_groups_ptr) {
                        for (int gsg = 0; gsg < gs_size_; ++gsg) {
                          ...
                          log.Log0Verbose1() << "SNIFF_TEST: Cell " << cell_->global_id << "  Angle_gidx=" << direction_num
                                               ...
                                               << psi_upwind_groups_ptr[gsg] << " Contrib_to_RHS_node_" << i_local_cell
                                               << "=" << (psi_upwind_groups_ptr[gsg] * mu_Nij_coeff);
                        }
                    }
                    else
                    {
        ...
        for (int gsg = 0; gsg < gs_size_; ++gsg) {
            ...
            for (unsigned int i_node = 0; i_node < cell_num_nodes_; ++i_node) {
              ...
              for (int m = 0; m < num_moments_; ++m) {
                ...
                const double current_m2d_op = groupset_.quadrature->GetMomentToDiscreteOperator()[m][direction_num];
                ...
                log.Log0Verbose1() << "SNIFF_TEST_QCALC: Cell " << cell_->global_id << " Ang=" << direction_num
                                       << " Node=" << i_node << " Mom=" << m << "  Calculated term=" << term;
                ...
                log.Log0Verbose1() << "SNIFF_TEST_QCALC: Cell " << cell_->global_id << " Ang=" << direction_num
                                       << " Node=" << i_node << " Mom=" << m << "  Updated q_angle_i=" << q_angle_i;
              }
              cell_source_for_angle[i_node] = q_angle_i;

              // Log the final result immediately after assignment
              log.Log0Verbose1() << "SNIFF_TEST_QCALC: Cell " << cell_->global_id << " Ang=" << direction_num
              << " Node=" << i_node << " FINAL cell_source_for_angle=" << cell_source_for_angle[i_node];
            }
             ...
             if (gs_size_ == 1) {
              for (unsigned int i_log = 0; i_log < cell_num_nodes_; ++i_log) {
                   log.Log0Verbose1() << "SNIFF_TEST: Cell " << cell_->global_id << "  Angle_gidx=" << direction_num << "  Grp=" << gsg
                                   << "    Node=" << i_log << " VolumetricSourceAngle_Before_MqLoop[" << i_log << "]=" << cell_source_for_angle[i_log];
               }
           }
        ...
        for (int m = 0; m < num_moments_; ++m) {
            const double wt_d2m_op_val = wt * groupset_.quadrature->GetDiscreteToMomentOperator()[m][direction_num];
            if (std::fabs(wt_d2m_op_val) < 1.0e-16) continue; // Skip if contribution is zero
        ...
        if (save_angular_flux_) {
            ...
            double* cell_psi_base_in_main_storage =
                &psi_destination_main[discretization_.MapDOFLocal(*cell_, 0, groupset_.psi_uk_man_, 0, 0)];

            for (unsigned int i_node = 0; i_node < cell_num_nodes_; ++i_node) {
                // Indexing for the main storage (uses global direction_num)
                const size_t node_block_offset_in_main_storage =
                    i_node * groupset_angle_group_stride_ +      // Offset to node i_node's psi data block
                    direction_num * groupset_group_stride_;     // Offset to global direction_num's data
                for (int gsg = 0; gsg < gs_size_; ++gsg) {
                    cell_psi_base_in_main_storage[node_block_offset_in_main_storage + gsg] = b_rhs[gsg](i_node);
                }
            }
        }
        ...
        for (unsigned int f = 0; f < cell_num_faces_; ++f) {
            ...
            if (is_boundary_outgoing && !is_reflecting_boundary_outgoing) {
                ...
                for (unsigned int fi_tally = 0; fi_tally < num_face_nodes_for_tally; ++fi_tally) {
                    ...
                    for (int gsg = 0; gsg < gs_size_; ++gsg) {
                        cell_transport_view_->AddOutflow(
                            f, gs_gi_ + gsg,
                            wt * face_mu_values[f] * b_rhs[gsg](i_node_tally) * IntF_shapeI_face_f(fi_tally)
                        );
                    }
                }
            }
            ...
            if (!is_local_outgoing && !is_boundary_outgoing) { // Outgoing to MPI neighbor
                ...
                size_t data_size_for_mpi_face = cell_mapping_->GetNumFaceNodes(f) * group_angle_stride_;
            ...
            for (unsigned int fi_on_face = 0; fi_on_face < num_face_nodes_on_f; ++fi_on_face) {
                const int i_node_in_cell = cell_mapping_->MapFaceNode(f, fi_on_face); // Cell node index
                ...
                if (is_local_outgoing) { // Write to compact CBC_FLUDS::local_psi_data_
                    ...
                    if (psi_target_in_compact_fluds) {
                        for (int gsg = 0; gsg < gs_size_; ++gsg) {
                          ...
                          log.Log0Verbose1() << "SNIFF_TEST: Cell " << cell_->global_id << "  Angle_gidx=" << direction_num
                                             ...
                                             << " Value=" << std::scientific << std::setprecision(6) << psi_target_in_compact_fluds[gsg];
                        }
                    }
                    ...
                    const size_t mpi_buffer_offset =
                        fi_on_face * (generic_angle_set.GetNumAngles() * gs_size_) + // Stride to current face node
                        angle_idx_as * gs_size_;                                  // Stride to current angle_as
                    
                    double* psi_target_in_mpi_buffer_ptr = &((*psi_dnwnd_mpi_buffer)[mpi_buffer_offset]);
                    for (int gsg = 0; gsg < gs_size_; ++gsg) {
