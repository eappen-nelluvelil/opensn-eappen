    ...
    for (const auto& dir_ss_info : dir_subsets)
    {
      ...
      else if (sweep_type_ == "CBC")
      {
        ...
        if (use_gpus_)
        {
          ...
          boundary_nonlocal_bytes = (cbcd_common_data.GetNumIncomingNonlocalNodes() +
        ...
        else
        {
          ...
          const auto num_groups_and_angles = gs_num_grps * angle_indices.size();
          boundary_nonlocal_bytes = cbc_common_data.GetTotalIncomingNonlocalFaceNodes() *
                                    num_groups_and_angles * sizeof(double);
        }
        ...
        {
          const auto& cbc_common_data = dynamic_cast<const CBC_FLUDSCommonData&>(fluds_common_data);
          cbc_fluds_local_psi_bytes.push_back(cbc_common_data.GetTotalLocalFaceSlotNodes() *
                                              gs_num_grps * angle_indices.size() * sizeof(double));
        }
