    ...
    for (size_t f = 0; f < data.cell_num_faces; ++f)
    {
      ...
      for (size_t fi = 0; fi < num_face_nodes; ++fi)
      {
        ...
        if (is_boundary_face)
        {
          ...
          for (size_t gsg = 0; gsg < data.gs_size; ++gsg)
            data.cell_outflow_view.Add(f, data.gs_gi + gsg, flux_i * b[gsg * NumNodes + i]);
        }

        double* psi = nullptr;
        if (is_local_face)
          psi = data.fluds.OutgoingPsi(data.cell_local_id, static_cast<unsigned int>(f), fi, as_ss_idx);
        else if (not is_boundary_face)
