...
CBC_Sweep_FixedN(CBCSweepData& data, AngleSet& angle_set)
{
  ...
  for (size_t as_ss_idx = 0; as_ss_idx < data.num_angles_in_as; ++as_ss_idx)
  {
    ...
    for (size_t f = 0; f < data.cell_num_faces; ++f)
    {
      ...
      const auto& cbc_fluds_common_data =
        static_cast<const CBC_FLUDSCommonData&>(data.fluds.GetCommonData());
      const bool is_delayed_local_face =
        is_local_face and cbc_fluds_common_data.IsDelayedLocalIncomingFace(
                            data.cell_local_id, static_cast<unsigned int>(f));
      const bool is_delayed_nonlocal_face = (not is_local_face) and (not is_boundary_face) and
                                            cbc_fluds_common_data.IsDelayedNonlocalIncomingFace(
                                              data.cell_local_id, static_cast<unsigned int>(f));
      ...
      for (size_t fj = 0; fj < num_face_nodes; ++fj)
      {
        ...
        if (is_local_face)
        {
          if (is_delayed_local_face)
            psi = data.fluds.DelayedLocalUpwindPsi(*data.cell_transport_view.FaceNeighbor(f),
        ...
        else if (not is_boundary_face)
        {
          if (is_delayed_nonlocal_face)
            psi = data.fluds.DelayedNLUpwindPsi(cbc_fluds_common_data.GetDelayedNonlocalFaceInfo(
                                                  data.cell_local_id, static_cast<unsigned int>(f)),
                                                face_nodal_mapping->face_node_mapping_[fj],
                                                as_ss_idx);
          else
            psi = data.fluds.NLUpwindPsi(data.cell.global_id,
    ...
    for (size_t f = 0; f < data.cell_num_faces; ++f)
    {
      ...
      const bool is_delayed_local_outgoing =
        is_local_face and cbc_fluds_common_data.IsDelayedLocalOutgoingFace(
                            data.cell_local_id, static_cast<unsigned int>(f));
      ...
      for (size_t fi = 0; fi < num_face_nodes; ++fi)
      {
        ...
        if (is_local_face)
        {
          if (is_delayed_local_outgoing)
            psi = data.fluds.DelayedLocalOutgoingPsi(data.cell, i, as_ss_idx);
          else
