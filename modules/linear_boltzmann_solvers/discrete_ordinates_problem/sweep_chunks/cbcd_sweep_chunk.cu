    ...
    time_dependent_(problem.IsTimeDependent())
{
  for (auto& as : *(groupset.angle_agg))
  {
    ...
    unsigned int stride_size =
      gpu_kernel::RoundUp(static_cast<unsigned int>(fluds->GetStrideSize()));
...
CBCDSweepChunk::Sweep(const std::vector<std::uint32_t>& cell_local_ids, std::size_t angle_set_id)
{
  ...
  auto grid_size_x = grid_size_x_list_[angle_set_id];
  auto grid_size_y = (num_ready_cells + block_size.y - 1) / block_size.y;
  crb::Dim3 grid_size(grid_size_x, grid_size_y);
  auto* host_cell_local_ids_data = host_cell_local_ids.data();
#if defined(__NVCC__) || defined(__HIPCC__)
  if (time_dependent_)
    gpu_kernel::SweepKernel<gpu_kernel::SweepType::CBC, true><<<grid_size, block_size, 0, stream>>>(
