...
CBCSweepChunkTD::Sweep(AngleSet& angle_set)
{
  ...
  // Compute time-absorption coefficients: tau_g = 1/(v_g * theta * dt)
  const auto& inv_velg = xs_.at(cell_->block_id)->GetInverseVelocity();
  const double theta = problem_.GetTheta();
  const double inv_theta = 1.0 / theta;
  const double dt = problem_.GetTimeStep();
  const double inv_dt = 1.0 / dt;

  std::vector<double> tau_gsg(gs_size_);
  for (size_t gsg = 0; gsg < gs_size_; ++gsg)
    tau_gsg[gsg] = inv_velg[gs_gi_ + gsg] * inv_theta * inv_dt;

  // Get pointer to psi_old for this cell
  const double* psi_old =
    &psi_old_[discretization_.MapDOFLocal(*cell_, 0, groupset_.psi_uk_man_, 0, 0)];
  ...
  for (size_t as_ss_idx = 0; as_ss_idx < num_angles_in_as_; ++as_ss_idx)
  {
    ...
    {
      // Time-dependent: sigma_t augmented with time-absorption coefficient
      double sigma_tg = sigma_t[gs_gi_ + gsg] + tau_gsg[gsg];
      ...
      for (size_t i = 0; i < cell_num_nodes_; ++i)
      {
        ...
        // Time-dependent RHS: add tau * psi_old contribution
    ...
    // If requested, save angular fluxes during sweep with theta-method reconstruction:
    ...
    if (SaveAngularFluxEnabled())
    {
      ...
      for (size_t i = 0; i < cell_num_nodes_; ++i)
      {
        ...
        for (size_t gsg = 0; gsg < gs_size_; ++gsg)
        {
          ...
          const double psi_old_val = psi_old[addr_offset + gsg];
