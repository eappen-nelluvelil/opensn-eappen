...
bool
IsCylindricalRZ(const AngleAggregation& angle_agg)
{
...
{
  return IsCylindricalRZ(angle_agg) && HasCurvilinearQuadrature(angle_agg);
}
...
SweepScheduler::InitializeAlgoDOG()
{
  ...
  const bool is_cylindrical_rz = IsCylindricalRZ(angle_agg_);
  ...
  if (is_cylindrical_rz && curvi_quad && product_quad)
  {
    int order = 0;
  ...
  for (size_t as = 0; as < num_anglesets; ++as)
  {
    ...
    if (loc_depth >= 0)
    {
      ...
      if (is_cylindrical_rz && !angle_order.empty() && angleset->GetNumAngles() == 1)
      {
