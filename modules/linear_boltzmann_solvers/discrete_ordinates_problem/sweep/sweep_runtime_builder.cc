...
{
  OpenSnLogicalErrorIf(bytes.size() % sizeof(CBC_SPDS::LocationEdgeWeight) != 0,
                 ...
                 const std::vector<int>& global_edges_to_remove)
{
  ...
  while (offset < global_edges_to_remove.size())
  {
    OpenSnLogicalErrorIf(offset + 2 > global_edges_to_remove.size(),
                         "Malformed CBC sweep FAS payload.");
    const auto spds_ordinal = static_cast<size_t>(global_edges_to_remove[offset++]);
    const auto num_edges = global_edges_to_remove[offset++];
    OpenSnLogicalErrorIf(num_edges < 0 or
                           offset + static_cast<size_t>(num_edges) > global_edges_to_remove.size(),
                         "Malformed CBC sweep FAS payload.");
    OpenSnLogicalErrorIf(spds_ordinal >= spds_list.size(),
                         "Invalid SPDS ordinal in the CBC sweep FAS payload.");
