...
public:
   ...
   *
   * Resets per-sweep task state and marks the angle set ready for batched execution.
   *
...
private:
  ...
  /// Boundary-bank offsets on host before upload.
  std::vector<std::uint64_t> host_boundary_offsets_;
  /// Boundary-bank offsets on device.
