...
//   // Create boundary wait node
//   cudaGraphNode_t boundary_wait_node;
//   cudaGraphAddEventWaitNode(&boundary_wait_node, 
//                             std::any_cast<cudaGraph_t>(cuda_graph_), 
//                             nullptr, 0, 
//                             boundary_event);
...
//   // Create kernel nodes WITH their dependencies
//   for (size_t i = 0; i < current_task_list.size(); ++i)
//   {
//     const auto& task = current_task_list[i];
//     const auto& args = graph_args[i];
...
//     cudaKernelNodeParams kernel_node_params{};
//     void* kernel_args[] = {const_cast<void*>(reinterpret_cast<const void*>(&args))};
//     kernel_node_params.func = (void*)cbc_gpu_kernel::CBCSweepKernel<cbc_gpu_kernel::GraphArguments>;
//     kernel_node_params.gridDim = dim3((args.batch_size + 127) / 128, 1, 1);
//     kernel_node_params.blockDim = dim3(128, 1, 1);
//     kernel_node_params.sharedMemBytes = 0;
//     kernel_node_params.kernelParams = kernel_args;
...
//     // Collect dependencies for THIS node
//     std::vector<cudaGraphNode_t> dependencies;
    
//     // Add boundary wait node if this cell has incoming boundary faces
//     if (task.has_incoming_boundary_faces)
//     {
//       dependencies.push_back(boundary_wait_node);
//     }

//     // Add predecessor cell nodes
//     for (const auto& pred_local_id : task.predecessors)
//     {
//       auto it = cell_local_id_to_node_map.find(pred_local_id);
//       if (it != cell_local_id_to_node_map.end())
//       {
...
//     cudaGraphNode_t kernel_node;
//     cudaGraphAddKernelNode(&kernel_node, 
//                           std::any_cast<cudaGraph_t>(cuda_graph_),
...
CBC_AngleSet::BuildAndInstantiateCUDAGraph(std::vector<cbc_gpu_kernel::GraphArguments>& graph_args)
{
  ...
  // PASS 1: Create all kernel nodes WITHOUT dependencies
  for (size_t i = 0; i < current_task_list.size(); ++i)
  {
    const auto& task = current_task_list[i];
    const auto& args = graph_args[i];
  ...
  // After Pass 1, verify all predecessors exist
  for (const auto& task : current_task_list)
  {
    for (const auto& pred_id : task.predecessors)
    {
      if (cell_local_id_to_node_map.find(pred_id) == cell_local_id_to_node_map.end())
      {
        opensn::log.Log() << "Task " << task.reference_id 
                                  << " has missing predecessor " << pred_id;
      }
    }
  }

  // PASS 2: Add all dependencies now that all nodes exist
for (const auto& task : current_task_list)
{
  cudaGraphNode_t current_node = cell_local_id_to_node_map[task.reference_id];
  std::vector<cudaGraphNode_t> from_nodes;

  // Add boundary wait dependency
  if (task.has_incoming_boundary_faces)
  {
    from_nodes.push_back(boundary_wait_node);
  }

  // Add predecessor dependencies (now ALL predecessors exist)
  for (const auto& pred_local_id : task.predecessors)
  {
    auto it = cell_local_id_to_node_map.find(pred_local_id);
    if (it != cell_local_id_to_node_map.end())
    {
      from_nodes.push_back(it->second);
    }
    else
    {
      // This should NEVER happen now - log error if it does
      opensn::log.Log() << "Missing predecessor node for cell " << pred_local_id;
    }
  }

  // Add edges: from_nodes -> current_node
  if (!from_nodes.empty())
  {
    // Create array of "to" nodes (all pointing to current_node)
    std::vector<cudaGraphNode_t> to_nodes(from_nodes.size(), current_node);
    
    cudaError_t err = cudaGraphAddDependencies(
      std::any_cast<cudaGraph_t>(cuda_graph_),
      from_nodes.data(),
      to_nodes.data(),
      nullptr,  // edgeData parameter - use nullptr for default
      from_nodes.size());
    
    if (err != cudaSuccess)
    {
      opensn::log.Log() << "Failed to add dependencies: " 
  ...
  // Instantiate
  cudaGraphExec_t graph_exec;
  cudaError_t err = cudaGraphInstantiate(&graph_exec, 
                                         std::any_cast<cudaGraph_t>(cuda_graph_), 
                                         nullptr, nullptr, 0);
  if (err != cudaSuccess)
  {
    opensn::log.Log() << "Graph instantiation failed: " 
                              << cudaGetErrorString(err);
  }
