    ...
    device_angle_indices_(angles_.size())
{
  ...
  cbcd_fluds_.GetCommonData().AddBoundaryOffsetsToAngleSet(*this, boundaries_);
  CopyBoundaryOffsetToDevice();
...
bool
CBCD_AngleSet::TryInitialize(CBCDSweepChunk&)
{
...
CBCD_AngleSet::SyncDeviceAngleIndices()
{
  ...
  crb::copy(device_angle_indices_, pin, angles_.size(), 0, 0, stream_);
}

void
CBCD_AngleSet::CopyBoundaryOffsetToDevice()
{
  ...
  crb::copy(device_boundary_offsets_, pin, host_boundary_offsets_.size(), 0, 0, stream_);
