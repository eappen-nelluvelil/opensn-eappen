...
//   // For problems with non-reflecting BCs, num_dependencies_ = 0, so latch(0)
//   // is immediately released and wait() returns without blocking
//   starting_latch_->wait();

//   // Copying incoming boundary psi to device
...
//     {
//       // Copy back outgoing reflecting boundary psi and non-local to host
...
//               current_task_list_[pred].num_dependencies)
//           {
...
//         // Copy incoming non-local psi for these cells to device
//         cbcd_fluds_.CopyIncomingNonlocalPsiToDevice(this, ready_cell_ids);

//         // Launch kernel on this angle set's stream
...
//         current_task_list_.begin(), current_task_list_.end(), [](const Task& t) { return t.completed; });
//     }
//   } // while not all_tasks_completed
...
//   // Update boundary readiness and notify following anglesets
...
CBCD_AngleSet::AngleSetAdvance(SweepChunk& sweep_chunk, AngleSetStatus permission)
{
  ...
  while (not all_tasks_completed)
  {
    ...
    if (kernel_in_flight and stream_.is_completed())
    {
      ...
      // Send MPI data staged by CopyOutgoingPsiBackToHost
      comm->SendData();
      in_flight_tasks.clear();
      in_flight_cell_ids.clear();
      kernel_in_flight = false;
