...
CBCD_AsynchronousCommunicator::CBCD_AsynchronousCommunicator(size_t angle_set_id,
                                                             FLUDS& fluds,
...
CBCD_AsynchronousCommunicator::SendData()
{
  ...
  // Batch queued payloads by destination location.
  if (not outgoing_message_queue_.empty())
  {
    ...
    for (const auto& [msg_key, data] : outgoing_message_queue_)
    {
      ...
      const uint64_t cell_global_id = msg_key.cell_global_id;
