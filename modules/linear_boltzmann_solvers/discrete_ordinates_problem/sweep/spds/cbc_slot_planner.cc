...
private:
  SlotSolveResult UseIdentityAssignment(std::vector<std::uint32_t>& face_slot_ids) const
  {
    ...
    for (std::uint32_t u_face = 0; u_face < num_faces_; ++u_face)
    {
      ...
      while (true)
      {
             ...
             ++v_face)
        {
          ...
          if (workspace_.face_mate_v[v_face] != INVALID_INDEX or
        ...
        if (not ConsumeFlowUnit(task_arc))
          return false;
      }
    }
