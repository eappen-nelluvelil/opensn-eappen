...
public:
  ...
  SlotSolveResult Solve(std::vector<std::uint32_t>& face_slot_ids)
  {
    ...
    {
      // An internally inconsistent decomposition must never reach CBC's compact storage.
      // Preserve the established conservative fallback: one immutable slot per directed face.
...
private:
  ...
  SlotSolveResult ExtractSlotAssignment(const std::size_t matching_size,
