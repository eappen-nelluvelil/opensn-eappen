    ...
    num_delayed_local_face_nodes_(0)
{
  ...
  std::size_t num_outgoing_nonlocal_faces = 0;
  ...
  std::vector<size_t> delayed_dependency_index_by_location(
    static_cast<size_t>(opensn::mpi_comm.size()), INVALID_FACE_SLOT);
  for (std::size_t i = 0; i < delayed_location_dependencies.size(); ++i)
  {
    const auto location = delayed_location_dependencies[i];
    delayed_dependency_index_by_location[static_cast<std::size_t>(location)] = i;
  }
  ...
  downstream_slot_by_face.reserve(num_outgoing_nonlocal_faces);
