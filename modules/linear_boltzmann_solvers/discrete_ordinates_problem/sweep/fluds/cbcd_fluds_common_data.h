...
public:
  ...
  /// Append this FLUDS map's boundary-bank offsets to an angle set.
  void AddBoundaryOffsetsToAngleSet(
    CBCD_AngleSet& angle_set,
    const std::map<std::uint64_t, std::shared_ptr<SweepBoundary>>& boundaries) const;

private:
  struct BoundaryOffsetInfo
  {
  ...
  /// Boundary nodes in the same order as boundary node indices.
  std::vector<BoundaryOffsetInfo> boundary_offset_infos_;
   ...
   * \param sdm Spatial discretization used to enumerate face nodes.
   */
  void CopyFlattenedNodeIndexToDevice(
    const SpatialDiscretization& sdm,
    const std::map<std::uint64_t, std::shared_ptr<SweepBoundary>>& boundaries);
