  ...
  const std::map<std::uint64_t, std::shared_ptr<SweepBoundary>>& boundaries)
{
  ...
  boundary_offset_infos_.reserve(total_face_nodes);
    ...
    for (size_t f = 0; f < cell.faces.size(); ++f)
    {
      ...
      for (size_t fn = 0; fn < num_face_nodes; ++fn)
      {
        ...
        if (is_incoming_face)
        {
          ...
          {
            const auto& boundary = *boundaries.at(face.neighbor_id);
            const auto boundary_offset_index =
              static_cast<std::uint64_t>(boundary_offset_infos_.size());
            node_index = CBCD_NodeIndex(boundary_offset_index,
                                        is_outgoing_face,
                                        boundary.IsReflecting(),
                                        boundary.IsAngleDependent());
            boundary_offset_infos_.push_back({face.neighbor_id,
          ...
          else
          {
            ...
            if (boundary.IsReflecting())
            {
              const auto boundary_offset_index =
                static_cast<std::uint64_t>(boundary_offset_infos_.size());
              node_index = CBCD_NodeIndex(
                boundary_offset_index, is_outgoing_face, true, boundary.IsAngleDependent());
              boundary_offset_infos_.push_back({face.neighbor_id,
                                                static_cast<std::uint32_t>(cell.local_id),
                                                static_cast<unsigned int>(f),
                                                static_cast<std::uint16_t>(fn),
              ...
              node_index = CBCD_NodeIndex(0, is_outgoing_face, false, false);
...
void
CBCD_FLUDSCommonData::AddBoundaryOffsetsToAngleSet(
  CBCD_AngleSet& angle_set,
  const std::map<std::uint64_t, std::shared_ptr<SweepBoundary>>& boundaries) const
{
  for (const auto& info : boundary_offset_infos_)
  {
    const auto boundary_it = boundaries.find(info.boundary_id);
    assert(boundary_it != boundaries.end());
    FaceNode fn(info.cell_local_id, info.face_id, info.face_node);
    angle_set.AddBoundaryOffset(
      boundary_it->second->GetOffsetToAngleset(fn, angle_set, info.is_outgoing));
  }
}
} // namespace opensn
