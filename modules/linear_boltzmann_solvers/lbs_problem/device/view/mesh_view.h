...
struct CellView
{
  ...
  __inline_host_dev__ void Update(const char* cell_data)
  {
    ...
    const double* const* inv_velocity_data = reinterpret_cast<const double* const*>(cell_data);
    inv_velocity = *(inv_velocity_data++);
    cell_data = reinterpret_cast<const char*>(inv_velocity_data);
    // phi address
