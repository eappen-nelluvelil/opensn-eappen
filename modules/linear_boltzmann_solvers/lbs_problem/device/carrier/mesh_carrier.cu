...
MeshCarrier::ComputeSize(LBSProblem& lbs_problem)
{
  ...
  for (const Cell& cell : mesh.local_cells)
  {
    ...
    // pointers to total cross sections and inverse velocity
  ...
  *(num_cell_data++) = num_cells;
  ...
  for (char* cell_data = data; const Cell& cell : mesh.local_cells)
  {
    ...
    *(total_xs_data++) = xs.GetXSGPUData(cell.block_id);
    ...
    double** inv_vel_data = reinterpret_cast<double**>(cell_data);
    *(inv_vel_data++) = xs.GetInvVelocityGPUData(cell.block_id);
    cell_data = reinterpret_cast<char*>(inv_vel_data);
    // phi address
