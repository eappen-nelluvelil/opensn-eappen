...
prepare_campaign()
{
  ...
  local cycles_sha=$(git -C "$SOURCE_ROOT" rev-parse HEAD)
  local trunk_sha=$(git -C "$SOURCE_ROOT" rev-parse refs/remotes/origin/main)
  local cycles_short=${cycles_sha[1,9]}
  local trunk_short=${trunk_sha[1,9]}
  local cycles_source=$OPENSN_DANE_WORK_ROOT/sources/cbc-cycles-$cycles_short
  local trunk_source=$OPENSN_DANE_WORK_ROOT/sources/trunk-$trunk_short
  local build_root=$OPENSN_DANE_WORK_ROOT/builds/$label
  local cycles_build=$build_root/cbc-cycles-$cycles_short-native
  local trunk_build=$build_root/trunk-$trunk_short-native

  mkdir -p "$OPENSN_DANE_WORK_ROOT/sources" "$OPENSN_DANE_WORK_ROOT/builds"
  local checkout expected actual
  for checkout expected in "$cycles_source" "$cycles_sha" "$trunk_source" "$trunk_sha"; do
    if [[ -e $checkout ]]; then
      actual=$(git -C "$checkout" rev-parse HEAD)
      if [[ $actual != $expected ]]; then
        print -u2 -- "ERROR: $checkout is at $actual, expected $expected"
        return 1
      fi
    else
      git -C "$SOURCE_ROOT" worktree add --detach "$checkout" "$expected"
    fi
  done
  ...
  python3 "$STUDY" prepare \
    ...
    --cycles-source "$cycles_source" \
    --cycles-sha "$cycles_sha" \
    --cycles-build "$cycles_build" \
    --trunk-source "$trunk_source" \
    --trunk-sha "$trunk_sha" \
    --trunk-build "$trunk_build" \
