...
class StudyTest(unittest.TestCase):
    def test_prepare_writes_both_complete_study_matrices(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            args = study.make_parser().parse_args(
                [
                    ...
                    "example",
                    "--cycles-source",
                    str(base / "cycles-source"),
                    "--cycles-sha",
                    "a" * 40,
                    "--cycles-build",
                    str(base / "cycles-build"),
                    "--trunk-source",
                    str(base / "trunk-source"),
                    "--trunk-sha",
                    "b" * 40,
                    "--trunk-build",
                    str(base / "trunk-build"),
                    "--geometry",
                    str(geometry),
                    "--cross-sections",
                    str(cross_sections),
    ...
    def test_parse_measurement_uses_final_timing_record(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = study.parse_measurement(stdout, "cycles", "strong", 2, 64, 1)
    ...
    def test_generated_jobs_are_native_and_use_64_ranks_per_node(self):
        ...
        job = study.make_study_job(manifest, "cycles", "strong", 4)
