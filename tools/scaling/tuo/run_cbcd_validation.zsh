...
Commands:
  ...
  submit-profiling
                 rebuild, prepare, and submit strong/weak pbatch diagnostics
  submit-campaign
...
show_configuration()
{
  ...
  print -- "profile_kinds=${OPENSN_TUO_PROFILE_KINDS:-strong}"
...
{
  unset OPENSN_CBCD_NUM_WORKERS
  export OPENSN_TUO_PROFILE_NODES=${OPENSN_TUO_PROFILE_NODES:-1,2,4,8,16,32,64,128,256}
  export OPENSN_TUO_PROFILE_KINDS=${OPENSN_TUO_PROFILE_KINDS:-strong,weak}
  export OPENSN_TUO_PROFILE_DIVISOR=${OPENSN_TUO_PROFILE_DIVISOR:-39}
  export OPENSN_TUO_PROFILE_ITERATIONS=${OPENSN_TUO_PROFILE_ITERATIONS:-10}
  export OPENSN_TUO_PROFILES=baseline,cbcd-metrics,caliper,pmpi,rocprof
  export OPENSN_TUO_ROCPROF_NODES=${OPENSN_TUO_ROCPROF_NODES:-1,2,4}
  ...
  campaign_paths
  [[ ! -e $campaign_profile_root ]] || {
    print -u2 -- "Profile label '$OPENSN_TUO_LABEL' already exists: $campaign_profile_root"
    exit 1
  }
  ...
  stage 'Preparing strong/weak CBCD profiling studies'
  zsh "$helper" prepare-profile

  stage 'Submitting baseline, internal, Caliper, and PMPI diagnostics'
  zsh "$campaign_profile_root/submit.zsh" \
    --nodes "$OPENSN_TUO_PROFILE_NODES" \
    --kinds "$OPENSN_TUO_PROFILE_KINDS" \
    --profiles baseline,cbcd-metrics,caliper,pmpi

  stage 'Submitting rank-zero rocprof diagnostics at selected node counts'
  zsh "$campaign_profile_root/submit.zsh" \
    --nodes "$OPENSN_TUO_ROCPROF_NODES" \
    --kinds "$OPENSN_TUO_PROFILE_KINDS" \
    --profiles rocprof

  stage 'Profiling jobs submitted'
...
submit_campaign()
{
  ...
  export OPENSN_TUO_PROFILES=baseline,cbcd-metrics,caliper-mpi,pmpi
