...
def profile_command(profile, nodes, ranks):
    ...
    if profile == "cbcd-metrics":
        setup = 'export OPENSN_CBCD_PROFILE_DIR="$result/cbcd-metrics"'
        return setup, f'{launch} "$binary" --verbose 1 -i "$input"'
    if profile == "caliper":
        command = (
...
def profile_job(args, study, profile, kind, nodes, input_path):
    ranks = 1 if profile == "omniperf" else nodes * 4
    result_root = study / "results" / profile / kind / f"nodes-{nodes}"
    scheduler = study / "scheduler"
    header = flux_header(
        f"{args.label}-{profile}-{kind[0]}-{nodes}",
        nodes,
        ranks,
        args.queue,
        args.bank,
        args.time_limit,
        scheduler / f"{profile}-{kind}-{nodes}-{{{{id}}}}.out",
        scheduler / f"{profile}-{kind}-{nodes}-{{{{id}}}}.err",
    )
    ...
    elif profile == "cbcd-metrics":
        artifact = (
            'find "$result/cbcd-metrics" -path "*/rank-*/sweeps.csv" '
...
def prepare_profile(args):
    ...
    unsupported = set(args.profile_nodes) - set(WEAK_DIVISORS)
    if "weak" in args.profile_kinds and unsupported:
        raise RuntimeError(f"no weak-scaling mesh is defined for nodes {sorted(unsupported)}")
    divisors = set()
    if "strong" in args.profile_kinds:
        divisors.add(args.profile_divisor)
    if "weak" in args.profile_kinds:
        divisors.update(WEAK_DIVISORS[node] for node in args.profile_nodes)
        if "omniperf" in args.profiles:
            divisors.add(WEAK_DIVISORS[1])
    meshes = required_meshes(args.mesh_dir, divisors)
    study = prepare_directory(args.output, args.refresh)
    ...
    for profile in args.profiles:
        ...
        for kind in args.profile_kinds:
            for nodes in nodes_values:
                divisor = (
                    args.profile_divisor if kind == "strong" else WEAK_DIVISORS[nodes]
                )
                input_path = study / "inputs" / f"{kind}-{nodes}.py"
                write_input(
                    input_path,
                ...
                job_path = study / "jobs" / f"{profile}-{kind}-{nodes}.zsh"
                write_executable(
                    job_path,
                    profile_job(args, study, profile, kind, nodes, input_path),
                )
                cases.append(
                    {
                        "id": f"{profile}-{kind}-{nodes}",
                        "profile": profile,
                        "kind": kind,
                        "nodes": nodes,
                        "ranks": 1 if profile == "omniperf" else nodes * 4,
...
def collect_cbcd_metrics(study, record):
    ...
    histogram_rows = []
    for case in record["cases"]:
        if case["profile"] != "cbcd-metrics":
            continue
        kind = case.get("kind", "strong")
        case_root = study / "results" / case["profile"] / kind / f"nodes-{case['nodes']}"
        for run in sorted(case_root.glob("run-*")):
            if not (run / "SUCCESS").is_file():
                continue
            sweep_rows = []
            for path in sorted((run / "cbcd-metrics").glob("rank-*/sweeps.csv")):
                with path.open(newline="") as stream:
                    sweep_rows.extend(csv.DictReader(stream))
            if not sweep_rows:
                continue

            def total(field):
                return sum(int(row[field]) for row in sweep_rows)

            def mean_ns(field):
                return total(field) / len(sweep_rows) / 1.0e9

            kernel_launches = total("kernel_launches")
            kernel_cells = total("kernel_cells")
            worker_wall_ns = total("worker_wall_ns")
            worker_idle_ns = total("worker_idle_ns")
            comm_iterations = total("comm_iterations")
            comm_idle_iterations = total("comm_idle_iterations")
            send_messages = total("send_messages")
            send_bytes = total("send_bytes")
            receive_messages = total("receive_messages")
            receive_bytes = total("receive_bytes")
            summaries.append(
                {
                    "kind": kind,
                    "nodes": case["nodes"],
                    "ranks": case["ranks"],
                    "run": run.name,
                    "rank_sweep_samples": len(sweep_rows),
                    "kernel_launches": kernel_launches,
                    "kernel_cells": kernel_cells,
                    "mean_cells_per_launch": (
                        kernel_cells / kernel_launches if kernel_launches else 0.0
                    ),
                    "minimum_cells_per_launch": min(
                        (
                            int(row["kernel_batch_min"])
                            for row in sweep_rows
                            if int(row["kernel_launches"]) > 0
                        ),
                        default=0,
                    ),
                    "maximum_cells_per_launch": max(
                        int(row["kernel_batch_max"]) for row in sweep_rows
                    ),
                    "worker_idle_fraction": (
                        worker_idle_ns / worker_wall_ns if worker_wall_ns else 0.0
                    ),
                    "worker_yields": total("worker_yields"),
                    "communicator_idle_fraction": (
                        comm_idle_iterations / comm_iterations
                        if comm_iterations
                        else 0.0
                    ),
                    "mean_flush_outgoing_s_per_rank_sweep": mean_ns(
                        "flush_outgoing_ns"
                    ),
                    "mean_probe_and_receive_s_per_rank_sweep": mean_ns(
                        "probe_and_receive_ns"
                    ),
                    "mean_poll_sends_s_per_rank_sweep": mean_ns("poll_sends_ns"),
                    "send_messages": send_messages,
                    "send_bytes": send_bytes,
                    "sent_face_records": total("send_faces"),
                    "mean_send_bytes": (
                        send_bytes / send_messages if send_messages else 0.0
                    ),
                    "receive_messages": receive_messages,
                    "receive_bytes": receive_bytes,
                    "received_face_records": total("receive_faces"),
                    "mean_receive_bytes": (
                        receive_bytes / receive_messages if receive_messages else 0.0
                    ),
                    "mean_communicator_drain_s": mean_ns("communicator_drain_ns"),
                    "maximum_communicator_drain_s": max(
                        int(row["communicator_drain_ns"]) for row in sweep_rows
                    )
                    / 1.0e9,
                    "mean_end_barrier_s": mean_ns("end_barrier_ns"),
                    "maximum_end_barrier_s": max(
                        int(row["end_barrier_ns"]) for row in sweep_rows
                    )
                    / 1.0e9,
            ...
            merged_histograms = {}
            for path in sorted((run / "cbcd-metrics").glob("rank-*/histograms.csv")):
                with path.open(newline="") as stream:
                    for row in csv.DictReader(stream):
                        ...
                        merged_histograms[key] = merged_histograms.get(key, 0) + int(
                            row["count"]
                        )
            for (metric, bin_id, lower, upper), count in sorted(merged_histograms.items()):
    ...
    write_rows(study / "cbcd-metrics-summary.csv", summaries)
    write_rows(study / "cbcd-metrics-histograms.csv", histogram_rows)
    if not summaries:
        return
    lines = [
        f"# {record['label']} CBCD internal metrics",
        "",
        (
            "| Kind | Nodes | Mean cells/launch | Worker idle | Communicator idle | "
            "Mean send (bytes) | Barrier max (s) | Drain max (s) | Run |"
        ),
        "|---|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for row in summaries:
        lines.append(
            f"| {row['kind']} | {row['nodes']} | {row['mean_cells_per_launch']:.4g} "
            f"| {row['worker_idle_fraction']:.2%} "
            f"| {row['communicator_idle_fraction']:.2%} "
            f"| {row['mean_send_bytes']:.4g} "
            f"| {row['maximum_end_barrier_s']:.4g} "
            f"| {row['maximum_communicator_drain_s']:.4g} "
            f"| `{row['result_directory']}` |"
        )
    (study / "cbcd-metrics-summary.md").write_text("\n".join(lines) + "\n")
    ...
    plots = (
        ...
        (
            "communicator_idle_fraction",
            "Communicator idle-poll fraction",
            "communicator-idle-fraction",
        ),
        ("mean_send_bytes", "Mean MPI send size (bytes)", "mpi-send-bytes"),
        ("maximum_end_barrier_s", "Maximum end-barrier wait (s)", "barrier-wait"),
        (
            "maximum_communicator_drain_s",
            "Maximum communicator drain (s)",
            "communicator-drain",
        ),
    )
    for field, ylabel, filename in plots:
        ...
        for kind in ("strong", "weak"):
            rows = sorted(
                (row for row in summaries if row["kind"] == kind),
                key=lambda row: int(row["nodes"]),
            )
            if rows:
                axis.plot(
        ...
        axis.legend()
        figure.tight_layout()
        figure.savefig(study / f"cbcd-{filename}.pdf")
        plt.close(figure)
...
def collect_profile(args):
    ...
    lines = [
        ...
        "",
        (
            "| Profile | Kind | Nodes | Ranks | Run | Completed | Sweep (s) | "
    ...
    for row in rows:
        ...
        lines.append(
            f"| {row['profile']} | {row['kind']} | {row['nodes']} | {row['ranks']} | {row['run']} "
...
def parser():
    ...
    profile_prepare.add_argument(
        "--profile-kinds", type=parse_kinds, default=("strong",)
    )
    profile_prepare.add_argument(
