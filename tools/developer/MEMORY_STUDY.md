...
default. CPU inputs retain `allow_cycles=True`. GPU inputs use
`allow_cycles=False`. Angular flux saving is disabled. The mesh and cross
...
No forced garbage collection is performed.
...
the launcher exit code are kept in the case directory. It does not infer success
from a timeout or suppress nonzero launcher exit codes. Use `run --trim` only for
...
backend, architecture, and dependency hints from an existing CMake cache. Source
...
These runs are diagnostics and must not be used as scaling-performance samples.
