#!/usr/bin/env bash
# Run a CBCD GPU regression test with Caliper's event-trace service, producing
# per-rank binary .cali traces suitable for offline analysis with cali-query.
#
# Usage:
#   run_trace.sh <test_script> [num_ranks] [output_dir]
#
# Defaults:
#   num_ranks  = 4
#   output_dir = ./cali_<test_stem>_<timestamp> under the test script directory
#
# Each MPI rank writes a separate .cali file into the output directory. Use
# summarize_cali.sh to aggregate counts and inclusive timings across ranks.
set -euo pipefail
...
# event-trace emits raw region enter/exit traces in .cali format. Use the
# service-managed output directory mode here; it is more reliable on this
# Caliper build than filename templating through the output= option.
...
if ! find "${OUT_DIR}" -maxdepth 1 -name "*.cali" -type f | read -r _; then
  echo "  (none — check cali_config)"
fi
echo
echo "[run_trace] summarize with: scripts/profiling/cbcd/summarize_cali.sh ${OUT_DIR}"
